...
每个 phase 复用同一布局：
- **Header**：project / run_id / 当前 phase / 当前 gate / Phoenix UI 链接
...
任何 Agent 不接观测三件套不允许进主线：**Phoenix tracing + loguru 结构化日志 + Pipeline 事件总线**。

### 11.1 Tracing · Phoenix

依赖：`arize-phoenix` + `openinference-instrumentation-anthropic`

在 `pipeline.py` 入口启动一次：
```python
...
效果：每次 `messages.create`、tool_use、tool_result、token 用量、耗时自动落 Phoenix span 树，Agent 业务代码**零侵入**。
...
每次状态机转移 / 产物验收 / User 介入 → 写 `events.jsonl` + 同步推 Phoenix span event。事后能完整重放整条流水线。

### 11.5 故障排查动线

1. **Phoenix UI**（`localhost:6006`）：trace 树看哪一步出错，点开看 prompt / response / tool args
